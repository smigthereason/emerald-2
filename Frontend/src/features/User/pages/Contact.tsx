...
import contactIcon from "/assets/icons/contact.png";
import { useAuth } from "../../../Shared/hooks/AuthContext"; // Assuming you have an auth hook
...
const Contact: React.FC = () => {
  ...
  const handleEnquiryClick = () => {
    setIsRightPanelActive(false);
    setFormData(prev => ({ ...prev, type: "enquiry" }));
  };
  ...
  return (
    <div className="flex items-center justify-center min-h-screen relative -top-20">
      ...
      >
        ...
        >
          <form onSubmit={handleSubmit} className="panel bg-white flex flex-col items-center justify-center h-full text-center px-12">
            <h1 className="font-bold text-2xl mb-2">Make an Enquiry</h1>
            <div className="social-container flex my-5">
              <img src={contactIcon} alt="contact" className="w-10 h-10" />
            </div>
        ...
        >
          <form onSubmit={handleSubmit} className="panel bg-white flex flex-col items-center justify-center h-full text-center px-12">
            ...
            {submitSuccess && (
              <p className="text-green-500 text-sm mt-2">Feedback submitted successfully!</p>
            )}
          </form>
        </div>
        ...
        >
          <div
            className={`panel bg-gradient-to-t from-[#d88598ab] to-[#e47a93] 
              text-white relative left-[-100%] h-full w-[200%]
              transition-transform duration-1000 ease-in-out
              ${isRightPanelActive ? "translate-x-1/2" : "translate-x-0"}`}
          >
